...
__all__ = ["getfilesystemencoding", "ou", "Error", "init_gstreamer", "common_options"]
...
def common_options():
    ...
    opts.add_option(
        "-f",
        "--force",
        help="Recalculate Replay Gain even if the "
        "file already contains gain information.",
        dest="force",
        action="store_true",
    )
    opts.add_option(
    ...
    opts.add_option(
        "-r",
        "--reference-loudness",
        help="Set the reference " "loudness to REF dB (default: %default dB)",
        metavar="REF",
        dest="ref_level",
        action="store",
        type="int",
    ...
    opts.add_option(
        "--mp3-format",
        help="Choose the Replay Gain data format "
    ...
    opts.set_defaults(force=False, dry_run=False, ref_level=89, mp3_format="default")
